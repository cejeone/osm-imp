var leafletControlGeocoder = (function (exports, L) {
                    ...
                    if (data.response.view && data.response.view.length) {
                         for (var i = 0; i <= data.response.view[0].result.length - 1; i++) {
                              ...
                              var bbox = L__namespace.latLngBounds(
                                   L__namespace.latLng(loc.mapView.topLeft.latitude, loc.mapView.topLeft.longitude),
                                   L__namespace.latLng(loc.mapView.bottomRight.latitude, loc.mapView.bottomRight.longitude)
                              );
                              results[i] = {
     ...
     function parseLatLng(query) {
          ...
          if ((match = query.match(/^([NS])\s*(\d{1,3}(?:\.\d*)?)\W*([EW])\s*(\d{1,3}(?:\.\d*)?)$/))) {
               ...
               (match = query.match(/^([NS])\s*(\d{1,3})°?\s*(\d{1,2})['′]?\s*(\d{1,3}(?:\.\d*)?)?["″]?\W*([EW])\s*(\d{1,3})°?\s*(\d{1,2})['′]?\s*(\d{1,3}(?:\.\d*)?)?["″]?$/))
          ) {
               ...
               (match = query.match(/^(\d{1,3})°?\s*(\d{1,2})['′]?\s*(\d{1,3}(?:\.\d*)?)?["″]\s*([NS])\W*(\d{1,3})°?\s*(\d{1,2})['′]?\s*(\d{1,3}(?:\.\d*)?)?["″]?\s*([EW])$/))
          ) {
     ...
     var Nominatim = /*#__PURE__*/ (function () {
          function Nominatim(options) {
               this.options = {
                    serviceUrl: "http://demo.inovamap.com/nominatim/",
          ...
          _proto.geocode = function geocode(query, cb, context) {
               ...
               getJSON(this.options.serviceUrl + "search.php", params, function (data) {
                    var results = [];
          ...
          _proto.reverse = function reverse(location, scale, cb, context) {
               ...
               getJSON(this.options.serviceUrl + "reverse.php", params, function (data) {
                    var result = [];
               ...
               L__namespace.geoJSON(data, {
                    ...
                    onEachFeature: function onEachFeature(feature, layer) {
                         ...
                         if (layer.getBounds) {
                              ...
                              bbox = L__namespace.latLngBounds(
                                   L__namespace.GeoJSON.coordsToLatLng(layer.feature.bbox.slice(0, 2)),
                                   L__namespace.GeoJSON.coordsToLatLng(layer.feature.bbox.slice(2, 4))
                              );
                         } else {
                              center = layer.getLatLng();
                              bbox = L__namespace.latLngBounds(center, center);
